...
- определить целевую модель хранения технологий конкретного предприятия и общего перечня обрабатываемых материалов: текущая ORM содержит `TechnologyType`, но не связь с `EnterpriseProfile`, а `Material` связан с профилем только косвенно через `Product` и `MaterialItem`; исторические имена `ProfileTechnology` и `ProfileMaterial` до этого решения не обозначают ORM-сущности;
